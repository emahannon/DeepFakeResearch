...
for i in lst:
	...
	while success:
		...
		if (length == 30 or (success == False and length > 0)):
			...
			for k in faces:
				...
				#minX = max((minX - 15), 0)
				#minY = max((minY - 15), 0)
				maxX = min((maxX + int(maxX*.10)), height)
				maxY = min((maxY + int(maxY*.10)), width)
				minX = max((minX - int(minX*.10)), 0)
				minY = max((minY - int(minY*.10)), 0)
				print(maxX)
				print(maxY)
				print(minX)
				print(minY)
				...
				cv2.imwrite("faceExtractTest/%s.jpg" % (name), img2)

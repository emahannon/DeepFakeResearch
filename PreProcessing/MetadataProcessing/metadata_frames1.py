...
with open('dfdc_train_part_1/metadata.json') as jsonFile:
...
for key in sortedKeys:
	...
	myKey = glob.glob('frames_part_1/' + str(keyNum) + '_*.jpg')
	#print(myKey)
	if (file[key]['label'] == "FAKE"):
		for name in myKey:
			shutil.copy(name, 'frames_fakes_1')
	if (file[key]['label'] == "REAL"):
		for name in myKey:
			shutil.copy(name, 'frames_reals_1')
	keyNum += 1;

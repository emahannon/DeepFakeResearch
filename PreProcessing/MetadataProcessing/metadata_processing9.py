...
with open('dfdc_train_part_9/metadata.json') as jsonFile:
...
for key in sortedKeys:
	...
	myKey = glob.glob('faces_part_9/' + str(keyNum) + '_*.jpg')
	#print(myKey)
	if (file[key]['label'] == "FAKE"):
		for name in myKey:
			shutil.copy(name, 'fakes_part_9')
	if (file[key]['label'] == "REAL"):
		for name in myKey:
			shutil.copy(name, 'reals_part_9')
	keyNum += 1;

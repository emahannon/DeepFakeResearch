...
with open('dfdc_train_part_12/metadata.json') as jsonFile:
...
for key in sortedKeys:
	...
	myKey = glob.glob('faces_part_12/' + str(keyNum) + '_*.jpg')
	#print(myKey)
	if (file[key]['label'] == "FAKE"):
		for name in myKey:
			shutil.copy(name, 'fakes_part_12')
	if (file[key]['label'] == "REAL"):
		for name in myKey:
			shutil.copy(name, 'reals_part_12')
	keyNum += 1;
